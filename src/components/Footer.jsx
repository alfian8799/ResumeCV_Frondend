            ...
            <footer className='flex flex-col justify-end bg-black pt-20 px-4 sm:px-6 lg:px-8 overflow-hidden w-full'>
                <div className='w-full max-w-7xl mx-auto'>
                    <div className="flex flex-wrap justify-between gap-y-12 lg:gap-x-8">
                        ...
                        <div className="w-full md:w-[45%] lg:w-[35%] flex flex-col items-start text-left">
                            ...
                            <p className="text-sm text-white/60 mt-6 max-w-87.5 leading-relaxed">
                                {isIndonesian ? 'VinzCV adalah platform modern untuk membantu Anda membuat CV yang terstruktur dan profesional secara otomatis dalam hitungan menit.' : 'VinzCV is a modern platform designed to help you build structured and professional CVs automatically in minutes.'}
                            </p>
                        </div>

                        <div className="w-[45%] md:w-[45%] lg:w-[15%] flex flex-col items-start text-left">
                            <h3 className="text-sm text-white font-medium">{isIndonesian ? 'Tautan Penting' : 'Important Links'}</h3>
                            <div className="flex flex-col gap-2 mt-6">
                                <a href="#" className='text-sm text-white/60 hover:text-white transition-colors'>Home</a>
                                <a href="#features" className='text-sm text-white/60 hover:text-white transition-colors'>{isIndonesian ? 'Fitur' : 'Features'}</a>
                                <a href="#testimoni" className='text-sm text-white/60 hover:text-white transition-colors'>{isIndonesian ? 'Testimoni' : 'Testimonials'}</a>
                                <a href="#cta" className='text-sm text-white/60 hover:text-white transition-colors'>{isIndonesian ? 'Kontak' : 'Contact'}</a>
                            </div>
                        </div>

                        <div className="w-[45%] md:w-[45%] lg:w-[15%] flex flex-col items-start text-left">
                            <h3 className='text-sm text-white font-medium'>{isIndonesian ? 'Media Sosial' : 'Social Links'}</h3>
                            <div className="flex flex-col gap-2 mt-6">
                        ...
                        <div className="w-full md:w-[45%] lg:w-[25%] flex flex-col items-start text-left mt-4 md:mt-0">
                            <h3 className='text-sm text-white font-medium'>{isIndonesian ? 'Berlangganan berita' : 'Subscribe for news'}</h3>
                            <div className="flex items-center border gap-2 border-white/20 h-13 max-w-80 w-full rounded-full overflow-hidden mt-4">
                                <input type="email" placeholder={isIndonesian ? 'Masukkan email...' : 'Enter your email..'} className="w-full h-full pl-6 outline-none text-sm bg-transparent text-white placeholder-white/60 placeholder:text-xs" required />
                                <button type="submit" className="bg-linear-to-b from-[#5623D8] to-[#7B53E2] hover:opacity-90 active:scale-95 transition w-56 h-10 rounded-full text-sm text-white cursor-pointer mr-1.5 focus:outline-none">{isIndonesian ? 'Langganan' : 'Subscribe'}</button>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-wrap sm:flex-row items-center justify-between gap-y-4 gap-x-2 relative z-10">
                        <p className='text-xs text-white/60'>© 2026 VinzCV. {isIndonesian ? 'Hak cipta dilindungi.' : 'All rights reserved.'}</p>
                        <div className="flex items-center gap-6 text-right">
                            <a href='#' className='text-xs text-white/60 hover:text-white transition-colors'>{isIndonesian ? 'Syarat & Ketentuan' : 'Terms & Conditions'}</a>
                            <div className='w-px h-4 bg-white/20'></div>
                            <a href='#' className='text-xs text-white/60 hover:text-white transition-colors'>{isIndonesian ? 'Kebijakan Privasi' : 'Privacy Policy'}</a>
                        </div>
                    </div>
